...
class ObjectDetection(Node):
    ...
    def callback(self, msg: PointCloud2) -> None:
        ...
        try:
            ...
            if cloud is None:
                self.get_logger().warn("DEBUG: from_ros_msg returned None")
                return

            # Filtered cloud for surface detection
            filtered_cloud_plane = self.filter_cloud(cloud, max_x_dist=0.5, min_height=-0.05, max_height=0.1)
            # Filtered cloud for object detection
            filtered_cloud_objects = self.filter_cloud(cloud, max_x_dist=0.5, min_height=-0.025, max_height=0.1)

            # Segmentation: Plane extraction
            plane_indices, plane_coefficients, plane_cloud = self.extract_plane(filtered_cloud_plane)

            # Clustering: Identify clusters corresponding to plane surfaces
            plane_clusters, surface_centroids, surface_dimensions = self.extract_clusters(plane_cloud, "Plane Surface")
            # Clustering: Identify clusters corresponding to objects placed on top of planes
            object_clusters, object_centroids, object_dimensions = self.extract_clusters(filtered_cloud_objects, "Object")

            # Publish the detected plane surface clusters as markers
    ...
    def extract_clusters(self, cloud: pcl.PointCloud, cluster_type: str) -> Tuple[List[pcl.PointCloud], List[List[float]], List[List[float]]]:
        """Extracts clusters corresponding to planes from the point cloud"""
        ...
        # Initialize lists to store plane clusters, centroids, and dimensions
        plane_clusters = []
        cluster_centroids = []
        cluster_dimensions = []
        ...
        if not plane_clusters:
            self.get_logger().warning(f"No {cluster_type} clusters extracted...")

        # Return the filtered plane clusters, centroids and cluster dimensions
        return plane_clusters, cluster_centroids, cluster_dimensions

    def pub_surface_marker(self, surface_centroids: List[List[float]], surface_dimensions: List[List[float]]) -> None:
        """Publishes the detected plane surface as cube markers"""
        ...
        if marker_array.markers:
            self.get_logger().info(f"Published {len(marker_array.markers)} plane surface markers")
            self.surface_pub.publish(marker_array)
        else:
            self.get_logger().warning("No plane surface markers to publish.")
